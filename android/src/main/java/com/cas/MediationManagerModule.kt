...
class MediationManagerModule(
  context: ReactApplicationContext,
  private val managerWrapper: MediationManagerWrapper
) : ReactContextBaseJavaModule(context), AdLoadCallback, AdCallback {
  ...
  companion object {
    private const val APP_OPEN_AD_TYPE = 3
  }
  ...
  private fun createJSAdCallback(module: String, callbackId: String): AdPaidCallback {
    return object : AdPaidCallback {
      ...
      override fun onShowFailed(message: String) {
        val ae = AdError(message) 
        val map = WritableNativeMap().apply {
          putString("callbackId", callbackId)
          putMap("error", WritableNativeMap().apply {
            putInt("code", ae.code)
            putString("message", ae.message)
          })
        }
        reactApplicationContext
          .getJSModule(DeviceEventManagerModule.RCTDeviceEventEmitter::class.java)
