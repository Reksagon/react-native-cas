...
#ifdef RCT_NEW_ARCH_ENABLED
@interface CASNativeAdView()<RCTCASNativeAdViewViewProtocol, MANativeAdDelegate, MAAdRevenueDelegate>
#else
@interface CASNativeAdView()<MANativeAdDelegate, MAAdRevenueDelegate>
#endif
...
// Callback to `CASNativeAdView.js`
...
@implementation CASNativeAdView
...
- (void)setupEventHandlers
{
    self.onAdLoadedEvent = [self](NSDictionary *event)
    {
        if ( _eventEmitter )
        {
            auto nativeAdViewEventEmitter = std::static_pointer_cast<CASNativeAdViewEventEmitter const>(_eventEmitter);
            
            NSDictionary *nativeAd = event[@"nativeAd"];
            NSDictionary *nativeAdImpl = event[@"nativeAdImpl"];
            
            CASNativeAdViewEventEmitter::OnAdLoadedEvent result =
            {
    ...
    self.onAdLoadFailedEvent = [self](NSDictionary *event)
    {
        if ( _eventEmitter )
        {
            ...
            CASNativeAdViewEventEmitter::OnAdLoadFailedEvent result =
            {
                .adUnitId = std::string([event[@"adUnitId"] ?: @"" UTF8String]),
    ...
    self.onAdClickedEvent = [self](NSDictionary *event)
    {
        if ( _eventEmitter )
        {
            ...
            CASNativeAdViewEventEmitter::OnAdClickedEvent result =
            {
                .adUnitId = std::string([event[@"adUnitId"] ?: @"" UTF8String]),
                .adFormat = std::string([event[@"adFormat"] ?: @"" UTF8String]),
                .networkName = std::string([event[@"networkName"] ?: @"" UTF8String]),
    ...
    self.onAdRevenuePaidEvent = [self](NSDictionary *event)
    {
        if ( _eventEmitter )
        {
            ...
            CASNativeAdViewEventEmitter::OnAdRevenuePaidEvent result =
            {
                .adUnitId = std::string([event[@"adUnitId"] ?: @"" UTF8String]),
                .adFormat = std::string([event[@"adFormat"] ?: @"" UTF8String]),
                .networkName = std::string([event[@"networkName"] ?: @"" UTF8String]),
...
{
    const auto &oldViewProps = *std::static_pointer_cast<CASNativeAdViewProps const>(_props);
    const auto &newViewProps = *std::static_pointer_cast<CASNativeAdViewProps const>(props);
...
- (nullable MANativeAdLoader *)adLoader
{
    ...
    {
        _adLoader = [[MANativeAdLoader alloc] initWithAdUnitIdentifier: self.adUnitId sdk: [CAS shared].sdk];
...
- (void)loadAd
{
    if ( ![CAS shared].sdk )
    {
        [[CAS shared] logUninitializedAccessError: @"CASNativeAdView.loadAd"];
        return;
    }
    
    if ( [self.isLoading compareAndSet: NO update: YES] )
    {
        [[CAS shared] log: @"Loading a native ad for Ad Unit ID: %@...", self.adUnitId];
    ...
    {
        [[CAS shared] log: @"Ignoring request to load native ad for Ad Unit ID %@, another ad load in progress", self.adUnitId];
    }
}
...
- (void)setTitleView:(NSNumber *)tag
{
    ...
    if ( !view )
    {
        [[CAS shared] log: @"Cannot find a title view with tag \"%@\" for %@", tag, self.adUnitId];
        return;
    }
...
- (void)setAdvertiserView:(NSNumber *)tag
{
    ...
    if ( !view )
    {
        [[CAS shared] log: @"Cannot find an advertiser view with tag \"%@\" for %@", tag, self.adUnitId];
        return;
    }
...
- (void)setBodyView:(NSNumber *)tag
{
    ...
    if ( !view )
    {
        [[CAS shared] log: @"Cannot find a body view with tag \"%@\" for %@", tag, self.adUnitId];
        return;
    }
...
- (void)setCallToActionView:(NSNumber *)tag
{
    ...
    if ( !view )
    {
        [[CAS shared] log: @"Cannot find a callToAction view with tag \"%@\" for %@", tag, self.adUnitId];
        return;
    }
...
- (void)setIconView:(NSNumber *)tag
{
    ...
    if ( !view )
    {
        [[CAS shared] log: @"Cannot find an icon image view with tag \"%@\" for %@", tag, self.adUnitId];
        return;
    }
...
- (void)setOptionsView:(NSNumber *)tag
{
    ...
    if ( !view )
    {
        [[CAS shared] log: @"Cannot find an option view with tag \"%@\" for %@", tag, self.adUnitId];
        return;
    }
...
- (void)setMediaView:(NSNumber *)tag
{
    ...
    if ( !view )
    {
        [[CAS shared] log: @"Cannot find a media view with tag \"%@\" for %@", tag, self.adUnitId];
        return;
    }
...
{
    [[CAS shared] log: @"Native ad loaded: %@", ad];
    ...
    if ( nativeAdView )
    {
        ...
        [[CAS shared] log: @"Native ad is of template type, failing ad load..."];
        self.onAdLoadFailedEvent([[CAS shared] adLoadFailedInfoForAd: self.adUnitId withError: nil]);
...
- (void)sendAdLoadedReactNativeEventForAd:(MANativeAd *)ad
{
    ...
    // 2. NativeAd for `CASNativeAdView.js` to render the views
    ...
    NSMutableDictionary *adInfo = [[[CAS shared] adInfoForAd: self.nativeAd] mutableCopy];
    adInfo[@"nativeAd"] = nativeAdInfo;
    adInfo[@"nativeAdImpl"] = jsNativeAd;
    
    // Send to `CASAdView.js`
...
- (void)didFailToLoadNativeAdForAdUnitIdentifier:(NSString *)adUnitIdentifier withError:(MAError *)error
{
    ...
    [[CAS shared] log: @"Failed to load native ad for Ad Unit ID %@ with error: %@", self.adUnitId, error];
    
    // Notify publisher
    self.onAdLoadFailedEvent([[CAS shared] adLoadFailedInfoForAd: adUnitIdentifier withError: error]);
}

- (void)didClickNativeAd:(MAAd *)ad
{
    self.onAdClickedEvent([[CAS shared] adInfoForAd: ad]);
}
...
{
    self.onAdRevenuePaidEvent([[CAS shared] adInfoForAd: ad]);
}
...
#ifdef RCT_NEW_ARCH_ENABLED
Class<RCTComponentViewProtocol> CASNativeAdViewCls(void)
{
    return [CASNativeAdView class];
}
#endif
